...
def create_labels_price_rise(df, col_name):
    """
...
class DataGenerator:
    def __init__(self, company_code, data_path='./stock_history', output_path='./outputs', strategy_type='original',
        ...
        self.BASE_URL = "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED" \
                        "&outputsize=full&apikey=" + api_key + "&datatype=csv&symbol="  # apikey
    ...
    def create_label_short_long_ma_crossover(self, df, short, long):
        """
    ...
    def create_features(self):
        ...
        if 'labels' not in df.columns or self.update:
            if re.match(r"\d+_\d+_ma", self.strategy_type):
                ...
                df['labels'] = self.create_label_short_long_ma_crossover(df, short, long)
            else:
    ...
    def feature_selection(self):
        ...
        topk = 350
        select_k_best = SelectKBest(k=topk)
    ...
    def get_rolling_data_next(self, start_date=None, cross_val_split=0.2):
        if not start_date:
            start_date = self.batch_start_date

        x_train, y_train, df_batch_train, sample_weights = self.get_data(start_date)
        train_end_date = df_batch_train.tail(1).iloc[0]["timestamp"]
        test_start_date = train_end_date + pd.offsets.DateOffset()
        test_end_date = test_start_date + pd.offsets.DateOffset()
        x_test, y_test, df_batch_test, _ = self.get_data(test_start_date)
        x_train, x_cv, y_train, y_cv, sample_weights, _ = train_test_split(x_train, y_train, sample_weights,
                                                                           train_size=1 - cross_val_split,
                                                                           test_size=cross_val_split, random_state=2,
                                                                           stratify=y_train)
        ...
        self.batch_start_date = self.batch_start_date + pd.offsets.DateOffset()
        is_last_batch = False
